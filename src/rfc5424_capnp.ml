...
let iter_non_empty_string ~f = function "" -> () | s -> f s
...
let build_pairs ~tydefs section tags =
  ...
  let create_pair :
      type a.
      Logs.Tag.set -> a Tag.typ -> Tag.tydef -> Logs.Tag.set * Pair.t option =
   fun tags typ tydef ->
    match Tag.find typ tydef tags with
    | None | Some (_, None) -> (tags, None)
    | Some (d, Some tagv) ->
...
let capnp_of_syslog
  ...
  iter_non_empty_string msg ~f:(Record.msg_set r) ;
  (* OVH needs this *)
...
let string_option_of_string = function "" -> None | s -> Some s

module SM = Map.Make (String)

let syslog_of_capnp r =
  let facility = Syslog_message.facility_of_int (Record.facility_get r) in
  let severity = Syslog_message.severity_of_int (Record.severity_get r) in
